...
class ArchivedComplaint(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            f"status IN ('{STATUS_NOT_RESOLVED}', '{STATUS_ACKNOWLEDGED}', '{STATUS_RESOLVED}')",
            name="ck_archived_complaints_status",
        ),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    original_complaint_id: Mapped[int] = mapped_column(Integer, unique=True, nullable=False, index=True)
    complaint_key: Mapped[str] = mapped_column(String(64), nullable=False, index=True)
