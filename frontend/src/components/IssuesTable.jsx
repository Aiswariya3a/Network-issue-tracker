  ...
  return (
    <div className="rounded-xl2 border border-cardBorder bg-white shadow-soft">
      <div className="divide-y divide-slate-100 md:hidden">
        {issues.map((issue) => (
          <article key={issue.complaint_key || issue.row_index} className="space-y-2 p-4">
            <div className="flex items-center justify-between gap-2">
              <p className="text-sm font-semibold text-slate-700">{issue.issue_type || "Issue"}</p>
        ...
        {issues.length === 0 ? <p className="p-6 text-center text-sm text-slate-500">No issues available.</p> : null}
      </div>
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {issues.map((issue) => (
              <tr key={issue.complaint_key || issue.row_index} className="transition hover:bg-slate-50">
                ...
                <td className="max-w-sm truncate px-4 py-3 text-slate-700">{issue.description || "-"}</td>
